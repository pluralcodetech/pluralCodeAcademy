...
import CommunityModal from '../Community/CommunityModal';
  ...
  const CourseLists = () => {
    ...
    const handleEditCourse = (courseData) => {
        ...
        // let query = Object.keys(courseData).map(key => key + '=' + courseData[key]).join('&');

        var query = Object.keys(courseData).map((key) => {
            return encodeURIComponent(key) + '=' + encodeURIComponent(courseData[key])
        }).join('&');
        history.push(`/update_course/${query}`)
    }
